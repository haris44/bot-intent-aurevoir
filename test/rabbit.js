...
test.beforeEach(() => {
    ...
    process.env.RABBIT_INTENT_AUREVOIR_QUEUE = 'aurevoir.queue'
    process.env.RABBIT_INTENT_AUREVOIR_BINDING = 'aurevoir.binding'
})

test(t => {
    const message = 'test'
    assertQueue(fakeConnexion, (msg) => t.deepEqual(message, msg.content))
    return fakeConnexion.then((conn) => conn.sendToConsumer('aurevoir.queue', '{ "content" : "' + message + '"}'))
})
